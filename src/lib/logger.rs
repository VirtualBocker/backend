...
impl Logger {
	pub fn message (&self, _type : logger_utils::MessageType, _msg : &String ) {
		...
		match _type {
			logger_utils::MessageType::Critical => eprintln!("\x1b[30m\x1b[41m{}\x1b[0m {date} {time} : {_msg}", _type.prefix()),
			logger_utils::MessageType::Debug	=> println! ("\x1b[36m{}\x1b[0m {date} {time} : {_msg}", _type.prefix()),
			logger_utils::MessageType::Error	=> eprintln!("\x1b[91m{}\x1b[0m {date} {time} : {_msg}", _type.prefix()),
			logger_utils::MessageType::Info		=> println! ("\x1b[35m{}\x1b[0m {date} {time} : {_msg}", _type.prefix()),
			logger_utils::MessageType::Warn		=> println! ("\x1b[33m{}\x1b[0m {date} {time} : {_msg}", _type.prefix()),
		};
	}
...
impl Logger { 
	pub fn new(date_format : logger_utils::DateFormat, time_format : logger_utils::TimeFormat) -> Self {
		Self {
			times : time_format,
			dates : date_format,
		}
